...
- **`scripts/antu-prune.sh`** — Layer 1 structural check; single writer of `wiki/meta/vault-report.json` (step 2)
- **`scripts/antu-validate-schema.sh`** — Validates `vault-report.json` schema drift, called by `antu-prune.sh` as a sibling
...
## Detection criteria — Layer 1 (antu-prune.sh)
