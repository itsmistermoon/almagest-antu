...
- **`scripts/antu-sanitize.sh`** — Detects and auto-redacts credentials in a temp file before ingestion (step 2)
- **`scripts/antu-index.py`** — Re-indexes vault embeddings after a new source is ingested (step 7)
...
2. **Download or read** — input is a URL, a `.raw/` file path, or pasted text:
   ...
   **Sanitization check** — before saving to `.raw/`, run `bash scripts/antu-sanitize.sh <temp-file>` (detects injection, exfiltration, and credential vectors) and inspect the JSON output:
...
- **Never reconstruct a credential redacted by `antu-sanitize.sh`** (step 2) — not even on explicit user insistence.
