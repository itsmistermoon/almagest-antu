...
- **Scripts live globally, never in consumer vaults.** `~/.cortex-forge/bin/hooks/` is the single runtime location — as of 2026-07-02 it holds only `cortex-reindex-post-commit.sh` (git hook), the agent lifecycle hook scripts having been removed. Consumer vaults (`moon-multivac`, `moon-academy`, etc.) have no `bin/` directory. The `sync` step in `cortex-forge-setup` only downloads `templates/*.md`; `bin/` is never synced to vault directories.
